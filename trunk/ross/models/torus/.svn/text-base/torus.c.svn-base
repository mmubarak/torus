...
torus_init(nodes_state * s, tw_lp * lp)
{
   ...
   */
  ts = 4 + tw_rand_exponential(lp->rng, MEAN_INTERVAL);
  //ts = MEAN_INTERVAL;
...
void
request_arrive(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  int i;
  tw_stime ts;
  tw_event *e;
  nodes_message *m;
    ...
    e = tw_event_new(msg->source, 1, lp);

    s->next_available_time += 1;    
    ...
    if ( s->node_queue_length[msg->source_direction][msg->source_dim]>=buffer )
      m->type = SEND;
    else
      m->type = SEND_DATA;
...
void
data_send(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  int i;
  tw_stime ts;
  tw_event *e;
...
void
req_send(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  int i;
  tw_lpid dst_lp;
  tw_stime ts;
  ...
  m->exponential_counter = msg->exponential_counter;
  m->src_lp = lp->gid;
...
void
req_arrive(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  int i;
  tw_stime ts;
  tw_event *e;
  ...
  if ( s->node_queue_length[msg->source_direction][msg->source_dim] >= buffer_size )
	...
	m->dest[i] = msg->dest[i];
      ...
      m->exponential_counter = 2 * msg->exponential_counter;
...
packet_process(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  ...
  if(lp->gid==msg->dest_lp)
    ...
    {
      e = tw_event_new(lp->gid, MEAN_PROCESS+4, lp);
      m = tw_event_data(e);
...
event_handler(nodes_state * s, tw_bf * bf, nodes_message * msg, tw_lp * lp)
{
  ...
  switch(msg->type)
    ...
    case PROCESS:
      ...
      // finite buffer
    case REQ_SEND:
      req_send(s,bf,msg,lp);
      break;
    case REQ_ARRIVE:
      req_arrive(s,bf,msg,lp);
      break;
    }
}
